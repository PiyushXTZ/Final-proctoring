...
EYE_AR_THRESH = 0.25  # Eye aspect ratio threshold for blinking
EYE_AR_CONSEC_FRAMES = 2  # Number of consecutive frames with low EAR = 1 blink
TOTAL_FRAMES = 10  # Number of frames to analyze
MOTION_THRESH = 2.0  # Motion threshold for real face
TEXTURE_THRESH = 80  # Laplacian variance threshold

image_queue = deque(maxlen=TOTAL_FRAMES)
motion_counter = 0
last_bbox = None
blink_frame_counter = 0  # For counting blink frames
...
def analyze_images():
    global motion_counter, blink_frame_counter

    blink_detected = False
    texture_spoof = False
    motion_detected = False
    blink_counter = 0  # Blinks detected across the frames
